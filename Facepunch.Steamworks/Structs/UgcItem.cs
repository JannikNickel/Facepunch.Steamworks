...
namespace Steamworks.Ugc
{
	public struct Item
	{
		...
		/// <summary>
		/// True if this is only visible with a link
		/// </summary>
		...
		public static async Task<string[]> BatchRequestPreviewImageUrls( PublishedFileId[] ids )
		{
			UGCQueryHandle_t handle;
			handle = SteamUGC.Internal.CreateQueryUGCDetailsRequest( ids, (uint)ids.Length );
			var result = await SteamUGC.Internal.SendQueryUGCRequest( handle );
			if ( result.HasValue == false || result.Value.Result != Result.OK )
			{
				return null;
			}
			...
			{
				if ( SteamUGC.Internal.GetQueryUGCPreviewURL( result.Value.Handle, i, out string preview ) )
				{
					urls[i] = preview;
				}
				else
